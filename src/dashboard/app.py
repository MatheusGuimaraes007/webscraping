...
# Título da aplicação
st.title('📊 Pesquisa de Mercado - Notebooks no Saldão da informática')
...
average_new_price = df['discounted_price'].mean()
...
col1, col2 = st.columns([4, 2])
df_non_zero_prices = df[df['discounted_price'] > 0]
average_price_by_brand = df_non_zero_prices.groupby('brand')['discounted_price'].mean().sort_values(ascending=False)
